    ...
    <div className={`h-screen w-screen overflow-hidden flex flex-col md:flex-row transition-colors duration-300`}>
      ...
      <aside className="hidden md:flex flex-col w-64 border-r ledger-border bg-[var(--bg-panel)] z-20 h-full">
          ...
          {navCategories.map((category) => {
            ...
            return (
              <div key={category.title} className="mb-4">
                ...
                >
                  <span className="text-[10px] font-mono ledger-muted uppercase tracking-widest group-hover:text-white transition-colors">{category.title}</span>
                  <span className="text-[10px] text-[var(--text-muted)] group-hover:text-white transition-colors">
                    {isOpen ? '[-]' : '[+]'}
                  </span>
                </button>
                      ...
                      return (
                        <button 
                          ...
                          className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-none border-l-4 transition-all duration-150 ${
                            active ? 'border-[var(--signal-amber)] bg-[var(--bg-hover)] text-[var(--signal-amber)]' : 'border-transparent text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]'
                          }`}
                        >
        ...
        <div className="p-4 border-t ledger-border">
          <div className="ledger-panel p-3 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-[var(--bg-void)] border ledger-border flex items-center justify-center font-mono font-bold text-xs">
                {user.name.charAt(0)}
              </div>
              <div className="overflow-hidden">
                <p className="font-bold text-xs truncate ledger-text">{user.name}</p>
                <p className="text-[9px] font-mono ledger-muted uppercase tracking-widest truncate">{user.role}</p>
              </div>
            </div>
          </div>
          <button onClick={handleLogout} className="w-full flex items-center justify-center space-x-2 p-2 btn-secondary border-transparent hover:border-[var(--border-hairline)]">
            <LogOut size={14} />
      ...
      {/* Mobile Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t ledger-border bg-[var(--bg-panel)] flex justify-around p-2 z-50 overflow-x-auto custom-scrollbar">
        {flatNavItems.filter(i => !i.adminOnly || user.role === 'Admin' || user.role === 'HR Manager').map((item, idx) => {
          const Icon = item.icon;
          const active = pathname === item.path;
          return (
            <button key={item.label + idx} onClick={() => router.push(item.path)} className={`p-3 flex flex-col items-center gap-1 min-w-[70px] ${active ? 'text-[var(--signal-amber)]' : 'text-[var(--text-muted)]'}`}>
              <Icon size={20} />
              <span className="text-[9px] font-mono uppercase tracking-widest truncate max-w-full">{item.label}</span>
            </button>
          )
        })}
      </nav>
