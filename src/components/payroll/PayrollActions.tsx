  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" onClick={exportPF} title="Provident Fund statement (CSV)">
          <PiggyBank className="h-4 w-4" /> PF
        </Button>
        <Button variant="outline" size="sm" onClick={exportGratuity} title="Gratuity accrual report (CSV)">
          <FileSpreadsheet className="h-4 w-4" /> Gratuity
        </Button>
        <Button variant="outline" size="sm" onClick={exportFull} title="Full payroll export (CSV)">
          <Download className="h-4 w-4" /> Export CSV
        </Button>
        <Button variant="primary" size="sm" onClick={() => setShowGenerate((s) => !s)}>
