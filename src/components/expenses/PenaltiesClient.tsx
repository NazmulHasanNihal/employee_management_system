  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card>
          <CardContent>
            <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-muted)]">{/* @ts-ignore */}<T>Unpaid Penalties</T></p>
            <p className="mt-2 text-fluid-3xl font-semibold text-[var(--rose)]">{formatCurrency(totalUnpaid, 'BDT', 'en')}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-muted)]">{/* @ts-ignore */}<T>Total Records</T></p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Ban className="h-4 w-4 text-[var(--rose)]" /> {/* @ts-ignore */}<T>Penalty Ledger</T></CardTitle>
        </CardHeader>
        <CardContent>
          {penalties.length === 0 ? (
            <p className="py-6 text-center text-sm text-[var(--text-muted)]">{/* @ts-ignore */}<T>You have no penalties. Keep up the great work! 🎉</T></p>
          ) : (
              ...
              {penalties.map((p: any) => (
                <div key={p.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--border-hairline)] bg-[var(--bg-hover)] p-4">
                  ...
                  <div className="flex items-center gap-2">
                    {p.status === 'PAID' ? (
                      <Badge variant="emerald" className="gap-1"><CheckCircle2 size={12} /> {/* @ts-ignore */}<T>Paid</T></Badge>
                    ) : (
                      <Badge variant="rose" className="gap-1"><AlertTriangle size={12} /> {/* @ts-ignore */}<T>Unpaid</T></Badge>
                    )}
                    {isAdmin && p.status !== 'PAID' && (
                      <button onClick={() => updatePenaltyStatus.mutate({ id: p.id, status: 'PAID' })} className="text-xs font-medium text-[var(--emerald)] hover:underline">{/* @ts-ignore */}<T>Mark Paid</T></button>
                    )}
                  </div>
                </div>
              ))}
